...
> You only need this if you want to rebuild `maiforge.exe` yourself.  
> A pre-built binary is already bundled at `maiforge/maiforge.exe`.
...
```bash
# 1. Clone the repo WITH submodules (MaiLib is a submodule under _thirdparty/)
git clone --recurse-submodules https://github.com/ryukiyomizuu/maiconverter.git
cd maiconverter

# If you already cloned without --recurse-submodules, run this instead:
git submodule update --init --recursive
```

```bash
# 2. Publish maiforge as a self-contained single-file exe (Windows x64)
dotnet publish maioconverter-custom/src/MaiConverter.Custom.Cli/MaiConverter.Custom.Cli.csproj ^
  -c Release ^
  -r win-x64 ^
  --self-contained true ^
  /p:PublishSingleFile=true ^
  -o maioconverter-custom/dist/win-x64
```
