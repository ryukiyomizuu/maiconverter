...
def run_crid_safe(cmd, cwd=None):
    """
    Run crid.exe via PowerShell Start-Process.
    Direct subprocess.run silently fails for crid.exe on Windows — the binary
    requires being launched without inherited stdio handles. Start-Process is
    the only reliable way to invoke it from Python.
    """
    if sys.platform != "win32":
        return run_subprocess_safe(cmd, cwd=cwd)

    exe = str(cmd[0])
    args = [str(a) for a in cmd[1:]]
    arg_list = ",".join(f'"{a}"' for a in args)
    cwd_part = f' -WorkingDirectory "{str(cwd)}"' if cwd else ""
    ps_cmd = (
        f'Start-Process -FilePath "{exe}" -ArgumentList {arg_list}'
        f'{cwd_part} -Wait -NoNewWindow'
    )
    return subprocess.run(
